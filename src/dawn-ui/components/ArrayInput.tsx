import { cloneElement, HTMLAttributes, ReactNode, useEffect, useState } from "react";
...
export default function ArrayInput(props: {
    ...
    function inputUpdated(event: React.ChangeEvent<HTMLInputElement>, key: number) {
    ...
    function removeInput(key: number) {
        setValues(oldValues => {
            const newValues = [...oldValues];
            newValues.splice(inputs.findIndex(x => x === key), 1);
        ...
        setInputs(oldInputs => {
            ...
            newInputs.splice(inputs.findIndex(x => x === key), 1);
            return newInputs;
        });
    }
